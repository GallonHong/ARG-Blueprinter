...
const TYPES = {
  ...
  Browse: {
    label: '浏览页',
    templates: ['2001 新闻', 'BBS 帖子', 'SCP 绝密文档', '遇害者手写日记'],
    fields: [
      ['siteName', '站点名称', '临江在线'],
      ['title', '页面标题', '案件详情'],
      ['date', '日期', '2001-07-17'],
...
function Inspector({ selected, nodes, edges, customTemplates = [], openTemplateModal, onClose, update, patch, remove, addRule, addNode }) {
  ...
  const applyPreset = preset => {
    patch(node => {
      node.fields.primaryColor = preset.primaryColor
      node.fields.bgColor = preset.bgColor
      node.fields.cardBg = preset.cardBg
      node.fields.textColor = preset.textColor
      node.fields.fontFamily = preset.fontFamily
    })
  }
        ...
        <div className="field">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {THEME_PRESETS.map((preset, idx) => (
              <button key={idx} className="ghost" style={{ fontSize: 11, padding: '7px 8px', textAlign: 'left', border: '1px solid #dce4ef' }} onClick={() => applyPreset(preset)}>
                {preset.name}
              </button>
            ))}
          </div>
